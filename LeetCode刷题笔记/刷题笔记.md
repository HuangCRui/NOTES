...
-  **k ~ i之间的数都被删完了**：一共是 i - k - 1 个数，**这是固定的**，产生值：w(k, i)

- 1 ~ k：随便删，还可以删除 j - (i - k - 1)个数，**这是不固定的**，**所以需要使这部分最大，且和后面没关系**，恰好是：f(k, j - (i - k - 1))
