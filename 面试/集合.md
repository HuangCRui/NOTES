...
*TreeSet*的底层是`TreeMap`,添加的数据存入了**map的key**的位置,而value则固定是**PRESENT(Object)**

使用红黑树的原理对新 add()的对象按照指定的顺序排序，每增加一个对象都会进行排序，**将对象插入到二叉树的指定位置**
...
- TreeSet 继承于`AbstractSet`，所以它是一个**Set集合**，具有**Set的属性和方法**。
- TreeSet 实现了`NavigableSet`接口，意味着它**<u>支持一系列的导航方法</u>**。比如查找与指定目标最匹配项。
- TreeSet 实现了`Cloneable`接口，意味着它能被克隆。
- TreeSet 实现了java.io.Serializable接口，意味着它支持序列化。
- TreeSet是基于TreeMap实现的，TreeSet对象的基础数据结构是TreeMap。
- TreeSet中的元素支持2种排序方式：自然排序 或者 根据创建TreeSet 时提供的 **Comparator实现** 进行排序。这取决于使用的构造方法。
...
![img](../picture/集合/2017515101101412.jpg)





- 有序的、没有重复元素的集合，通过TreeMap实现

- TreeSet中含有一个"**NavigableMap类型**的成员变量"m，而m实际上是"TreeMap的实例"。
  - NavigableMap是一个接口，提供一些导航方法
  - ![image-20221009090201288](../picture/集合/image-20221009090201288.png)
...
从以上可知，实际上**一个链表被放满8个节点的概率非常小**，实际上链表转红黑树是非常耗性能的，而链表大小在8个节点以内的平均查询复杂度（O(4)，常量，平均）与红黑树(最大log8=3)相差无几，超过8个节点，黑红树的查询复杂度会好一些。所以，当链表的节点大于等于8个的时候，转为红黑树的性价比比较合适。
...
- 一方面是优化散列算法
- 另一方面是值域范围扩大。**如：HashMap保证容量为2 ^ n，非常有效的减少了冲突几率**
...
数组 + 链表的结构。Java7中是数组 + 链表，Java8 以后当链表长度大于 8 时会转换成红黑树



- 散列表：顾名思义，快速查找元素 ==> **数组随机访问**。数组的元素：Entry节点，能存储Key，Value。通过key计算hashcode，hashcode对数组长度取余得到数组下标，通过下标去寻找数组对应的value
- 从key的hashcode转换到数组下标是一个 **压缩映射**，不同的key计算出的hashcode可能相同，从而造成哈希冲突。HashMap采用的是**拉链法**。（还有一种开放地址法）
- 在 Java8 之前，使用链表，查找复杂度O(n)，引入红黑树，二叉平衡搜索树，查找时间只和树的高度有关，**树的高度为 log n，复杂度为：O(log n)**，时间复杂度**超级降低**
...
开放地址法容易出现数据堆积，**面对较大数据量的情况下**，**可能会出现<u>连续冲突</u>的情况，性能变得不稳定**。

- 线性探测：效率很低，产生非常长的**探测长度**，后续数据插入非常费时

- >   线性探测就是使用算术取余的方法计算余数，当产生冲突时就通过线性递增的方法进行探测，一直到数组的位置为空，插入数据项即可。

- > 二次探测：二次探测是过程是x+1,x+4,x+9,以此类推。二次探测的步数是原始位置相隔的步数的平方。
  >
  >  二次探测可以消除在线性探测中产生的聚集问题，但是二次探测还是会产生一种**<u>更明确更细的聚集</u>**。二次聚集的产生是在二次探测的基础上产生的现象。例如N个数据经hash函数计算后都映射到到数组下标10，探测第二个数字需要以一步长，第三个数字需要以4步长为单位，第四个数字则需要以九为步长......................
  >
  > **解决聚集问题**还是有一种更好的办法：再哈希法。

- 再哈希法：把关键字用不同的哈希函数再做一遍哈希化，用这个结果作为步长，**对指定的关键字，探测的步长是不变的**，可以说**不同的关键字可以使用不同的步长**，并且步长可以控制。一般来说，再哈希函数可以采用以下这种：
  ...
  public int hasFunction(int key){
  	return 5 - key % 5; //此处设置探测步长为1 ~ 5，如果设置为0，则会死循环，又因为key一样，步长一直为0，一直在原地，
  }
