...
class Solution {
    ...
    /*
    前序遍历的第一个节点是当前子树的根节点，可以在中序遍历中找到这个根节点，根节点的左右两侧是左右子树，递归
    */
    public TreeNode buildTree(int[] preorder, int[] inorder) {
        if(preorder == null || preorder.length == 0){
            return null;
        }
        indexMap = new HashMap<Integer, Integer>(inorder.length);
        //将中序遍历的结果和下标写入map，不用扫描，直接读取到根节点在中序遍历中的位置，确定左右子树
    ...
    public TreeNode build(int[] preorder, int[] inorder, int a, int b, int x, int y){
        ...
        //找到当前根节点在中序遍历的位置
        int idx = indexMap.get(preorder[a]);

        //左子树的节点个数
        int leftNum = idx - x;
        //递归左子树的区间，最后返回其根节点
        root.left = build(preorder, inorder, a + 1, a + leftNum, x, idx - 1);
        //递归右子树的区间，最后返回其根节点
        root.right = build(preorder, inorder, a + leftNum + 1, b, idx + 1, y);
...
class Solution {
    public int minArray(int[] nums) {
        ...
        /*
        [10,1,10,10,10]
        在第一次循环后，l=1,r=4，这时[l,r]本身就是有序的了！就会出现mid和l比较出现的三种情况之一（本身有序），但是比较r和mid就不会出现这种情况，所以此处需要特别判断一下！
...
直接看三叶的题解！三叶！yyds！https://leetcode-cn.com/problems/make-the-xor-of-all-segments-equal-to-zero/solution/gong-shui-san-xie-chou-xiang-cheng-er-we-ww79/
...
**其实两个数的异或就是他们不同的位。统计 x ^ y 二进制中的 1 的个数**
...
但是碰到不合法的位置就不能再移动过去了，那么(1,0)和(10,0)就不是连通的，即不可到达
...
第二个序列中的每个元素在第一个序列中的位置，第一个序列中的元素是不重复的，每个元素在第一个序列中出现的位置是唯一的。

A和B的公共子序列LCS，A和  B'  的上升子序列LIS，
...
qi ：当前所有长度是i的上升子序列的**最小末尾值**

qi - i的函数是**严格递增**的（证明）

新元素x，x可以更新哪些值？末尾>=x，x就不能接到末尾

比x小的第一个元素qk，将x接到k后面，比k小的其余的序列，把x接到其后面没有意义（取q i+1和x的min。qk<x，qi+1<=qk, q1+1 < x, x只能放在k后面，更新k+1），

只能够更新比x小的最大的元素，k+1一定大于等于x（因为长度为k的序列的末尾值 是第一个小于x的元素），更新长度为k+1的序列的末尾为x。
