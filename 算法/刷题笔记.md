...
Trie 是一颗非典型的**多叉树模型**，多叉好理解，即**每个结点的分支数量可能为多个**。
...
class TrieNode{
	...
	TrieNode[26] next    //字母映射表
}
```

可以看到***TrieNode结点中并没有直接保存字符值的数据成员，那它是怎么保存字符的呢？***

这时字母映射表next 的妙用就体现了，TrieNode* next[26]中保存了**对当前结点而言**下一个可能出现的**所有字符的链接**，因此我们可以**通过一个父结点来预知它所有子结点的值**：
