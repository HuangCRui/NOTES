...
class Solution {
    ...
    public boolean search(char[][] board, String word, int x, int y, int index, boolean[][] flag){
        ...
        for(int i = 0; i < 4; i++){
            int nx = x + dirs[i][0];
            int ny = y + dirs[i][1];
            // if(nx == -1 || nx == board.length || ny == -1 || ny == board[0].length || flag[nx][ny] == true){
            //     continue;
            // }
            if(nx >= 0 && nx <= board.length-1 && ny >= 0 && ny <= board[0].length-1 && flag[nx][ny] == false){
                if(search(board, word, nx, ny, index+1, flag)){
                    //这里不能忘了
                     flag[x][y] = false;
                    return true;
                }
