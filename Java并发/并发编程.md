...
> 线程**必须持有**对象的锁
>
> 调用wait()方法时，会***  释放 ***当前的锁，直到有其他线程调用notify(All)()唤醒等待
> 锁 的线程
>
...
> 重排：
>
> - 第一个操作是volatile读，第二个操作是普通读。如果进行重排序：先普通读，再volatile读，**在volatile读的时候会刷新缓存行**。会导致什么问题呢：给变量赋值时，先给volatile变量赋值，再给普通变量赋值。但是这时候缓存行只有volatile读的值，没法给普通变量赋值，需要重新去从主存中找
