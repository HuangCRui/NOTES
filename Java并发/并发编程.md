...
-   [进程与线程](#进程与线程)
    -   [上下文切换](#上下文切换)
-   [Java多线程入门类和接口](#java多线程入门类和接口)
    -   [Thread类和Runnable接口](#thread类和runnable接口)
    -   [Thread类构造方法](#thread类构造方法)
    -   [Thread类和Runnable接口的比较](#thread类和runnable接口的比较)
    -   [Callable、Future与FutureTask](#callablefuture与futuretask)
-   [线程组和线程优先级](#线程组和线程优先级)
    -   [线程的优先级](#线程的优先级)
    -   [线程组的常用方法及数据结构](#线程组的常用方法及数据结构)
-   [Java线程的状态及主要转化方法](#java线程的状态及主要转化方法)
    -   [Java线程的6个状态](#java线程的6个状态)
    -   [线程状态的转换](#线程状态的转换)
-   [Java线程间的通信](#java线程间的通信)
    -   [锁与同步](#锁与同步)
    -   [等待/通知机制](#等待通知机制)
    -   [信号量](#信号量)
    -   [管道](#管道)
    -   [其他通信相关](#其他通信相关)
-   [Java内存模型基础知识](#java内存模型基础知识)
    -   [并发编程模型的两个关键问题](#并发编程模型的两个关键问题)
    -   [Java内存模型的抽象结构](#java内存模型的抽象结构)
    -   [既然堆是共享的，为什么在堆中会有内存不可见问题？](#既然堆是共享的为什么在堆中会有内存不可见问题)
-   [重排序与happens-before](#重排序与happens-before)
    -   [重排序](#重排序)
    -   [顺序一致性模型与JMM的保证](#顺序一致性模型与jmm的保证)
    -   [happens-before](#happens-before)
-   [volatile](#volatile)
    -   [禁止重排序](#禁止重排序)
    -   [volatile阻止重排序](#volatile阻止重排序)
    -   [volatile的用途](#volatile的用途)
-   [synchronized与锁](#synchronized与锁)
    -   [生产者-消费者问题](#生产者-消费者问题)
...
-   [CAS与原子操作](#cas与原子操作)
    -   [乐观锁与悲观锁](#乐观锁与悲观锁)
    -   [CAS的概念](#cas的概念)
    -   [Java实现CAS的原理 - Unsafe类](#java实现cas的原理---unsafe类)
    -   [原子操作-AtmoicInteger类源码](#原子操作-atmoicinteger类源码)
    -   [CAS实现原子操作的三大问题](#cas实现原子操作的三大问题)
    -   [ABA问题](#aba问题)
    -   [CAS只能保证一个共享变量的原子操作](#cas只能保证一个共享变量的原子操作)
-   [AQS](#aqs)
    -   [AQS的数据结构](#aqs的数据结构)
    -   [资源共享模式](#资源共享模式)
    -   [AQS的主要方法源码解析](#aqs的主要方法源码解析)
        -   [获取资源](#获取资源)
        -   [释放资源](#释放资源)
-   [线程池原理](#线程池原理)
    -   [线程池的原理](#线程池的原理)
        -   [ThreadPoolExecutor提供的构造方法](#threadpoolexecutor提供的构造方法)
        -   [ThreadPoolExecutor 的策略](#threadpoolexecutor-的策略)
        -   [线程池主要的任务处理流程](#线程池主要的任务处理流程)
        -   [ThreadPoolExecutor如何做到线程复用的？](#threadpoolexecutor如何做到线程复用的)
    -   [四种常见的线程池](#四种常见的线程池)
...
-   [阻塞队列](#阻塞队列)
    -   [阻塞队列的由来](#阻塞队列的由来)
    -   [BlockingQueue的操作方法](#blockingqueue的操作方法)
    -   [BlockingQueue的实现类](#blockingqueue的实现类)
    -   [阻塞队列的原理](#阻塞队列的原理)
    -   [示例和使用场景](#示例和使用场景)
        -   [生产者-消费者模型](#生产者-消费者模型)
        -   [线程池中使用阻塞队列](#线程池中使用阻塞队列)
-   [锁接口和类](#锁接口和类)
    -   [synchronized的不足之处](#synchronized的不足之处)
    -   [锁的几种分类](#锁的几种分类)
        -   [可重入锁的非可重入锁](#可重入锁的非可重入锁)
        -   [公平锁与非公平锁](#公平锁与非公平锁)
        -   [读写锁和排它锁](#读写锁和排它锁)
    -   [JDK中有关锁的一些接口和类](#jdk中有关锁的一些接口和类)
        -   [抽象类AQS/SQLS/AOS](#抽象类aqssqlsaos)
        -   [接口Condition/Lock/ReadWriteLock](#接口conditionlockreadwritelock)
...
-   [CopyOnWrite容器](#copyonwrite容器)
    ...
    -   [CopyOnWrite的业务中实现](#copyonwrite的业务中实现)
-   [通信工具类](#通信工具类)
    -   [Semaphore](#semaphore)
...
-   [Fork/Join框架](#forkjoin框架)
    -   [工作窃取算法](#工作窃取算法)
    -   [Fork/Join 的具体实现](#forkjoin-的具体实现)
        -   [ForkJoinTask](#forkjointask)
        -   [ForkJoinPool](#forkjoinpool)
            -   [runState](#runstate)
    -   [Fork/Join的使用](#forkjoin的使用)
-   [Stream并行计算原理](#stream并行计算原理)
    -   [Stream单线程串行计算](#stream单线程串行计算)
    -   [Stream多线程并行计算](#stream多线程并行计算)
    -   [从源码看Stream并行计算原理](#从源码看stream并行计算原理)
    -   [Stream并行计算的性能提升](#stream并行计算的性能提升)
-   [计划任务](#计划任务)
    -   [类结构](#类结构)
    -   [主要方法介绍](#主要方法介绍)
        -   [ScheduledFutureTask类](#scheduledfuturetask类)
        -   [scheduledAtFixedRate](#scheduledatfixedrate)
        -   [delayedExecute](#delayedexecute)
    -   [DelayedWorkQueue](#delayedworkqueue)
        -   [take](#take)
        -   [offer](#offer)
    -   [总结](#总结)
-   [详解Condition的await和signal等待/通知机制](#详解condition的await和signal等待通知机制)
-   [小问题](#小问题)
    -   [java.lang.IllegalMonitorStateException异常](#java.lang.illegalmonitorstateexception异常)
    -   [stop()和suspend()方法为何不推荐使用](#stop和suspend方法为何不推荐使用)
    -   [Synchronized原理是什么](#synchronized原理是什么)
    -   [获取对象的锁，这个"锁"到底是什么？如何确定对象的锁？](#获取对象的锁这个锁到底是什么如何确定对象的锁)
    -   [什么是可重入性，为什么说synchronized是可重入锁？](#什么是可重入性为什么说synchronized是可重入锁)
        -   [线程安全](#线程安全)
        -   [可重入](#可重入)
        -   [可重入与线程安全](#可重入与线程安全)
        -   [synchronized的可重入性](#synchronized的可重入性)
        -   [synchronized可重入锁的实现](#synchronized可重入锁的实现)
...
**调用start()
方法**后，该线程才算启动。**虚拟机**创建一个线程，然后等到这个线程**第一次得到时间片**，会**调用run()**方法。
...
**start()方法只能Thread及其子类使用，用Runnable实现类来构造一个Thread对象
并start()**
...
内存可见性，指的是当一个线程对volatile修饰的变量进行
**写操作**时，JMM会立即把该线程对应的 **本地内存**中的
**共享变量的值刷新到主内存**；
...
*`<u>`{=html}这里面
a和flag都是共享变量，只是更新普通变量a的时候不会将其立马刷新到主内存，而更新flag时
会一同将a刷新到主内存中`</u>`{=html}*
...
1.  组织屏障两侧的指令重排序
2.  强制把
    写缓冲区/高速缓存中的脏数据等写回主内存，或者让缓存中响应的数据失效
...
-   **LoadLoad屏障**：对于这样的语句 Load1; LoadLoad;
    Load2，在Load2及后续读取操作要读取的数据被访问前，**保证Load1要读取的数据被读取完毕**。
-   **StoreStore屏障**：对于这样的语句Store1; StoreStore;
    Store2，在Store2及后续写入操作执行前，这个屏障会**把Store1强制刷新到内存**，保证Store1的**写入操作对其它处理器可见**。
    -   在写入volatile变量之前就需要刷新一次内存
-   **LoadStore屏障**：对于这样的语句Load1; LoadStore;
    Store2，在Store2及后续写入操作被刷出前，保证Load1要读取的数据**被读取完毕**。
-   **StoreLoad屏障**：对于这样的语句Store1; StoreLoad;
    Load2，在Load2及后续所有读取操作执行前，**保证Store1的写入对所有处理器可见**。它的开销是四种屏障中最大的（**冲刷写缓冲器，清空无效化队列**）。在大多数处理器的实现中，这个屏障是个万能屏障，**兼具其它三种内存屏障的功能**
...
> 如果进行重排序，那么volatile写
> 会使其他CPU的缓存行无效，不能保证volatile写之前的共享变量数据的一致。
>
> 在volatile
> 变量进行**读**操作的时候，会直接从**主存**中读取，再存储到**缓存行**。
...
当把共享变量声明为 volatile
类型后，线程对该变量修改时会将该变量的值**立即刷新回主内存**，同时会使其它线程中缓存的该变量**无效**，从而其它线程在读取该值时会从主内存中**重新读取**该值（参考缓存一致性）。因此在读取
volatile 类型的变量时总是会返回最新写入的值。
...
如果线程A执行了1
3还没开始执行2，线程B执行到了`if(singleton == null)`，它会判定singleton不为空，然后直接返回了一个**未初始化完成**的singleton
...
对锁 置个变量，如果发现是true，代表**无资源竞争**，则**无需**再走各种
加锁/解锁流程。
...
线程在第一次进入同步块时，会在**对象头**和 **栈帧**中的
**锁记录**里存储**锁的偏向的线程ID**。

当下次**该线程进入**这个同步块时，回去检查锁的mark
word里面是不是放的自己的线程ID。

如果是，表明该线程**已经获得了锁**，以后该线程在进入和退出同步块时**不需要花费CAS操作来加锁和解锁**
；如果不是，就代表**有另一个线程来竞争这个偏向锁**。这个时候会尝试使用CAS来**替换**Mark
Word里面的**线程ID为新线程的ID**(*替换为这个新线程的ID*)，这个时候要分两种情况：

-   成功，表示**之前的线程不存在了**，mark
    word里面的**线程ID为*新*线程的ID**，锁不会升级，仍然为偏向锁
-   失败，表示**之前的线程仍然存在**，那么**暂停**之前的线程，设置偏向锁标识为0，并设置锁标志位为00，**升级为轻量级锁**，会按照轻量级锁的方式**进行竞争锁**。

> CAS: Compare and Swap
>
> 比较并设置。用于在硬件层面上提供原子性操作。在 Intel
> 处理器中，比较并交换通过指令cmpxchg实现。
...
图中涉及到了lock record指针指向当前堆栈中的**最近一个lock
record**，是轻量级锁按照**先来先服务**的模式进行了**轻量级锁的加锁**。
...
线程1 将要执行的对象 的 对象头中的mark word 里保存的线程1的线程id
设置为空 此时 **那个对象** 不是 **偏向锁** 而是 **无锁**
...
JVM会为每个线程在当前**线程的栈帧**中创建用于***存储锁记录***的空间：**Displaced
Mark Word**。如果一个线程获得锁的时候发现是**轻量级锁**，会把锁的Mark
Word**复制**到自己的**Displaced Mark Word**里面。

线程尝试用CAS**将锁的mark
word替换为指向锁记录的指针**，成功：当前线程获得锁，失败：mark
word已经被替换成了其他线程的锁记录，说明在与其他线程竞争锁，当前线程尝试使用
**自旋** 来获取锁
...
当前线程使用CAS将displased Mark Word的内容 复制回 Mark
Word里面，如果**没有发生竞争**，那么复制的操作会成功。如果有其他线程因为自旋多次导致轻量级锁**升级成了重量级锁**，那么CAS**操作会失败**，此时会**释放锁并唤醒被阻塞的线程**。
...
*`<u>`{=html}判断V是否等于E，如果等于，将V的值设置为N；如果不等，说明已经有其它线程更新了V，则当前线程放弃更新，什么都不做。`</u>`{=html}*
...
不会的不会的不会的！
**`<u>`{=html}因为CAS是一种原子操作，它是一种系统原语，是一条CPU的原子指令，从CPU层面保证它的原子性。`</u>`{=html}**

**当多个线程同时使用CAS操作一个变量时，`<u>`{=html}只有一个会胜出`</u>`{=html}，并成功更新，其余均会失败，但失败的线程并`<u>`{=html}不会被挂起`</u>`{=html}，仅仅是被告知失败，并且允许再次尝试，当然也允许失败的线程放弃操作**
...
AQS本身实现的是一些排队和阻塞的机制，比如：具体线程等待**队列的维护**（获取资源失败
入队 /
唤醒出队）。先进先出的双端队列，两个指针head和tail用于标识队列的头部和尾部。
...
获取资源的入口是acquire(int
arg)方法。arg是要获取的资源的个数，在独占模式下始终未1.
...
-   如果获取资源失败 (**就还需要继续判断后面的条件，执行后面的动作**)
    ，就通过addWaiter(Node.EXCLUSIVE)方法把这个线程插入到等待队列中。传入的参数代表要插入的Node是独占式的。
...
***一个类中的同步方法调用
(本类的)另一个同步方法***，假如synchronized不支持重入，进入method2方法时当前**线程获得锁**，method2方法里面执行method1时当前线程**又要去尝试获取锁**，这时如果不支持重入，它就要**等释放，把自己阻塞**，导致自己锁死自己
